...
import { FullClassDefinition } from "./Parser/LogicParser";

export class GraphGenerator {
  static async render(str: string) {
    return (await instance()).renderSVGElement(str);
  }
}
...
export class UMLGenerator {
  async generate(from: Record<string, FullClassDefinition>) {
    ...
    result += "}";
    console.log(result);
    return GraphGenerator.render(result);
  }
  ...
  private processSpecifiers(cls: FullClassDefinition) {
    ...
    cls.members.forEach((mem) => {
      ...
      members += `<tr><td align="left">${currentAccess} ${
        mbr.member_type
      } ${esc(mbr.owner_modifier ?? "")}${mbr.member_name}${cm}</td></tr>\n`;
    });

    cls.methods.forEach((mem) => {
      const mth = mem.method;
      currentAccess = translate[mem.access];
      methods += `<tr><td align="left">${currentAccess}${mth.return_type} ${
        mth.method_name
      }(${mth.parameters
        .flatMap((param) => {
  ...
  private generateLinks(
    classes: Record<string, FullClassDefinition>,
    cls: FullClassDefinition
  ) {
    let ret = "";
